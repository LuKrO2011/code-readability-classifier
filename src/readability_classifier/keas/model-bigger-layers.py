...
# │ (InputLayer)        │ 3)                │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ struc_conv1         │ (None, 48, 303,   │        640 │ struc_reshape[0]… │
# │ (Conv2D)            │ 64)               │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
...
# │ (MaxPooling2D)      │ 64)               │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ struc_conv2         │ (None, 22, 149,   │     73,856 │ struc_pool1[0][0] │
# │ (Conv2D)            │ 128)              │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ seman_bert          │ (None, 100, 768)  │ 23,425,536 │ seman_input_toke… │
# │ (BertEmbedding)     │                   │            │ seman_input_segm… │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ vis_conv2 (Conv2D)  │ (None, 64, 64,    │     36,928 │ vis_pool1[0][0]   │
# │                     │ 64)               │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
...
# │ (MaxPooling2D)      │ 64)               │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ struc_conv3         │ (None, 11, 74,    │    295,168 │ struc_pool2[0][0] │
# │ (Conv2D)            │ 256)              │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ seman_pool1         │ (None, 32, 64)    │          0 │ seman_conv1[0][0] │
# │ (MaxPooling1D)      │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ vis_conv3 (Conv2D)  │ (None, 32, 32,    │     73,856 │ vis_pool2[0][0]   │
# │                     │ 128)              │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ struc_pool3         │ (None, 3, 24,     │          0 │ struc_conv3[0][0] │
# │ (MaxPooling2D)      │ 256)              │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ seman_conv2         │ (None, 28, 64)    │     20,544 │ seman_pool1[0][0] │
# │ (Conv1D)            │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ vis_pool3           │ (None, 16, 16,    │          0 │ vis_conv3[0][0]   │
# │ (MaxPooling2D)      │ 128)              │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ struc_flatten       │ (None, 18432)     │          0 │ struc_pool3[0][0] │
# │ (Flatten)           │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ seman_gru           │ (None, 128)       │     66,048 │ seman_conv2[0][0] │
# │ (Bidirectional)     │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ vis_flatten         │ (None, 32768)     │          0 │ vis_pool3[0][0]   │
# │ (Flatten)           │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ concatenate         │ (None, 51328)     │          0 │ struc_flatten[0]… │
# │ (Concatenate)       │                   │            │ seman_gru[0][0],  │
# │                     │                   │            │ vis_flatten[0][0] │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ class_dense1        │ (None, 64)        │  3,285,056 │ concatenate[0][0] │
# │ (Dense)             │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
...
# │ (Dense)             │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ class_dense3        │ (None, 1)         │         17 │ class_dense2[0][… │
# │ (Dense)             │                   │            │                   │
# └─────────────────────┴───────────────────┴────────────┴───────────────────┘
# Total params: 27,526,305 (105.00 MB)
# Trainable params: 27,526,305 (105.00 MB)
...
#
# freeze:
# 2024-10-02 07:29:35,147,147 root INFO Overall results:
# Best validation acc score: 0.9523809523809523
# Best validation precision score: 1.0
# Best validation recall score: 0.9090909090909091
# Best validation f1 score: 0.9523809523809523
# Best validation auc score: 0.9545454545454546
# Best validation mcc score: 0.9090909090909091
# Average validation acc score: 0.8142857142857143
# Average validation precision score: 0.8178768453768454
# Average validation recall score: 0.836096681096681
# Average validation f1 score: 0.8149044224628035
# Average validation auc score: 0.8269867632367631
