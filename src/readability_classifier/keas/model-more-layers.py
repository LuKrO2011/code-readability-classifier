...
# │ (InputLayer)        │ 3)                │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ struc_conv1         │ (None, 48, 303,   │        320 │ struc_reshape[0]… │
# │ (Conv2D)            │ 32)               │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ vis_conv1 (Conv2D)  │ (None, 128, 128,  │        896 │ vis_input[0][0]   │
# │                     │ 32)               │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ struc_pool1         │ (None, 24, 151,   │          0 │ struc_conv1[0][0] │
# │ (MaxPooling2D)      │ 32)               │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
...
# │ (MaxPooling2D)      │ 32)               │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ struc_conv2         │ (None, 22, 149,   │      9,248 │ struc_pool1[0][0] │
# │ (Conv2D)            │ 32)               │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ seman_bert          │ (None, 100, 768)  │ 23,425,536 │ seman_input_toke… │
# │ (BertEmbedding)     │                   │            │ seman_input_segm… │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ vis_conv2 (Conv2D)  │ (None, 64, 64,    │      9,248 │ vis_pool1[0][0]   │
# │                     │ 32)               │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ struc_pool2         │ (None, 11, 74,    │          0 │ struc_conv2[0][0] │
# │ (MaxPooling2D)      │ 32)               │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ seman_conv1         │ (None, 96, 32)    │    122,912 │ seman_bert[0][0]  │
# │ (Conv1D)            │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ vis_pool2           │ (None, 32, 32,    │          0 │ vis_conv2[0][0]   │
# │ (MaxPooling2D)      │ 32)               │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ struc_conv3         │ (None, 9, 72, 32) │      9,248 │ struc_pool2[0][0] │
# │ (Conv2D)            │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ seman_pool1         │ (None, 32, 32)    │          0 │ seman_conv1[0][0] │
# │ (MaxPooling1D)      │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ vis_conv3 (Conv2D)  │ (None, 32, 32,    │      9,248 │ vis_pool2[0][0]   │
# │                     │ 32)               │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ struc_pool3         │ (None, 4, 36, 32) │          0 │ struc_conv3[0][0] │
# │ (MaxPooling2D)      │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ seman_conv2         │ (None, 28, 32)    │      5,152 │ seman_pool1[0][0] │
# │ (Conv1D)            │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ vis_pool3           │ (None, 16, 16,    │          0 │ vis_conv3[0][0]   │
# │ (MaxPooling2D)      │ 32)               │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ struc_conv4         │ (None, 2, 34, 64) │     18,496 │ struc_pool3[0][0] │
# │ (Conv2D)            │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ seman_pool2         │ (None, 9, 32)     │          0 │ seman_conv2[0][0] │
# │ (MaxPooling1D)      │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ vis_conv4 (Conv2D)  │ (None, 16, 16,    │     18,496 │ vis_pool3[0][0]   │
# │                     │ 64)               │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ struc_pool4         │ (None, 1, 17, 64) │          0 │ struc_conv4[0][0] │
# │ (MaxPooling2D)      │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ seman_conv3         │ (None, 5, 32)     │      5,152 │ seman_pool2[0][0] │
# │ (Conv1D)            │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ vis_pool4           │ (None, 8, 8, 64)  │          0 │ vis_conv4[0][0]   │
# │ (MaxPooling2D)      │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ struc_flatten       │ (None, 1088)      │          0 │ struc_pool4[0][0] │
# │ (Flatten)           │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ seman_gru           │ (None, 64)        │     16,640 │ seman_conv3[0][0] │
# │ (Bidirectional)     │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ vis_flatten         │ (None, 4096)      │          0 │ vis_pool4[0][0]   │
# │ (Flatten)           │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ concatenate         │ (None, 5248)      │          0 │ struc_flatten[0]… │
# │ (Concatenate)       │                   │            │ seman_gru[0][0],  │
# │                     │                   │            │ vis_flatten[0][0] │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ class_dense1        │ (None, 64)        │    335,936 │ concatenate[0][0] │
# │ (Dense)             │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ class_dropout       │ (None, 64)        │          0 │ class_dense1[0][… │
# │ (Dropout)           │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ class_dense2        │ (None, 16)        │      1,040 │ class_dropout[0]… │
# │ (Dense)             │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ class_dense3        │ (None, 8)         │        136 │ class_dense2[0][… │
# │ (Dense)             │                   │            │                   │
# ├─────────────────────┼───────────────────┼────────────┼───────────────────┤
# │ class_dense4        │ (None, 1)         │          9 │ class_dense3[0][… │
# │ (Dense)             │                   │            │                   │
# └─────────────────────┴───────────────────┴────────────┴───────────────────┘
# Total params: 23,987,713 (91.51 MB)
# Trainable params: 23,987,713 (91.51 MB)
# Non-trainable params: 0 (0.00 B)

# Freeze
# 2024-10-03 11:40:15,856,856 root INFO Overall results:
# Best validation acc score: 0.9047619047619048
# Best validation precision score: 0.9090909090909091
# Best validation recall score: 0.9090909090909091
# Best validation f1 score: 0.9090909090909091
# Best validation auc score: 0.9090909090909091
# Best validation mcc score: 0.8090909090909091
# Average validation acc score: 0.8142857142857143
# Average validation precision score: 0.854477466977467
# Average validation recall score: 0.7646212121212121
# Average validation f1 score: 0.7884238373712057
# Average validation auc score: 0.8095493395493396

# No Freeze
# 2024-10-03 11:45:49,748,748 root INFO Overall results:
# Best validation acc score: 0.9523809523809523
# Best validation precision score: 0.8333333333333334
# Best validation recall score: 1.0
# Best validation f1 score: 0.9090909090909091
# Best validation auc score: 0.9166666666666667
# Best validation mcc score: 0.8838834764831843
# Average validation acc score: 0.8095238095238095
# Average validation precision score: 0.8258886946386946
# Average validation recall score: 0.8181818181818181
# Average validation f1 score: 0.8065582442335598
# Average validation auc score: 0.8220352564102565
...
def create_classification_layers(input_layer: tf.Tensor) -> tf.Tensor:
    ...
    dense3 = layers.Dense(units=8, activation="relu", name="class_dense3")(dense2)
    return layers.Dense(1, activation="sigmoid", name="class_dense4")(dense3)
...
def create_structural_extractor(
    ...
    conv3 = layers.Conv2D(
        filters=32, kernel_size=3, activation="relu", name="struc_conv3"
    )(pool2)
    pool3 = layers.MaxPooling2D(pool_size=2, strides=2, name="struc_pool3")(conv3)

    # Fourth convolutional block
    conv4 = layers.Conv2D(
        filters=64, kernel_size=3, activation="relu", name="struc_conv4"
    )(pool3)
    # pool_size and strides = 3 does not work with the given input shape
    pool4 = layers.MaxPooling2D(pool_size=2, strides=2, name="struc_pool4")(conv4)

    # Flatten the output
    flattened = layers.Flatten(name="struc_flatten")(pool4)
...
def create_semantic_extractor(
    ...
    # Third convolutional block
    conv3 = layers.Conv1D(32, 5, activation="relu", name="seman_conv3")(pool2)

    # Bidirectional LSTM
    gru = layers.Bidirectional(
        layers.LSTM(32, name="seman_lstm", return_sequences=False), name="seman_gru"
    )(conv3)
...
def create_visual_extractor(
    ...
    # Third convolutional block
    conv3 = layers.Conv2D(
        filters=32, kernel_size=3, padding="same", activation="relu", name="vis_conv3"
    )(pool2)
    pool3 = layers.MaxPooling2D(pool_size=2, strides=2, name="vis_pool3")(conv3)

    # Fourth convolutional block
    conv4 = layers.Conv2D(
        filters=64, kernel_size=3, padding="same", activation="relu", name="vis_conv4"
    )(pool3)
