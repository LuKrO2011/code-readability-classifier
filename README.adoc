...
- https://wkhtmltopdf.org/downloads.html[Windows/MacOS]: After downloading and installing, make sure to add the `/bin` folder to your PATH.
...
4. (Optional) If you plan on training the model, use https://docs.github.com/en/repositories/working-with-files/managing-large-files/installing-git-large-file-storage[git lfs] to download the datasets:
[source,bash]
git lfs install
git lfs pull
